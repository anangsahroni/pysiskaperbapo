...
class EJPERBO:
    ...
    def _single_query(self, payload, market_data):
        with requests.session() as rs:
            ...
            data['HARGA_SEKARANG'] = data['HARGA_SEKARANG'].str.replace('.','',regex=False)
            data['PERUBAHAN_RP'] = data['PERUBAHAN_RP'].str.replace('.','',regex=False)
            data['PERUBAHAN_PERSEN'] = data['PERUBAHAN_PERSEN'].str.replace(',','.')
            data['PERUBAHAN_PERSEN'] = data['PERUBAHAN_PERSEN'].str.replace('%','')
